...
def compute_skyline(img_filename):
	...
	img_name = img_filename[8:-4]
	img = cv2.imread(os.getcwd() + img_filename)
	...
	## generating the edgemap
	edgeness_filename = '/mats/' + img_name + '.mat'
	...
	cv2.imwrite(os.getcwd() + '/images_skylines/' + img_name + ".png", image)
	print('Image with skyline successfully saved')


def main():

	img_filename = '/images/img_1.png'
	compute_skyline(img_filename)
